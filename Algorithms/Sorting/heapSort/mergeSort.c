...
void merge(int arr[], int temp[], int l, int m, int r) {
	int i, j, k;
    /*
     * i is the starting point for the first sorted array
     * j is the starting point for the second sorted array
     * k is the starting point for the temp array
     */
	i = l;
	j = m + 1;
	k = l;
	while(i <= m && j <= r) {
        /*
         * merge the value from two sorted array
         * and copy it to the temp array
         * so the temp array is the sorted
         */
		if (arr[i] <= arr[j]){
			temp[k] = arr[i];
			i++;
	...
	while(i <= m) {
        /*
         * check if there are elements left in the first array
         * if there are, copy the elements into temp
         */
		temp[k] = arr[i];
		i++;
		k++;
	...
	while(j <= r) {
         ...
         * if there are, copy the elements into the temp
         */
		temp[k] = arr[j];
		j++;
		k++;
	...
	for (int n = l; n <= r; ++n) {
        /*
         * temp now is the sorted array
         * copy the value from temp arr
         *
         */
		arr[n] = temp[n];
	}
...
void mergeSort(int arr[], int temp[],int l, int r){
     ...
     mergeSort(arr,temp,l,mid);//sort the left part
     mergeSort(arr,temp,mid+1,r);//sort the right part
     merge(arr,temp,l,mid,r);//merge them all together
	}
}
...
void sortIntegers(int* array, unsigned int size){
    ...
    int* temp=(int*)malloc(sizeof(int)*size);//allocate enough space for the temp
	mergeSort(array, temp, 0, size - 1);//call function
